...
mi(phrase(RuleSet,Input), Memory, Context, Session, Params) :-
  phrase_input(Input),
  phrase_input([]),
  call_dcg__(RuleSet, Input, [], Memory, Context, Session, Params).


mi(phrase(RuleSet,Input,Rest), Memory, Context, Session, Params) :-

  phrase_input(Input),
  phrase_input(Rest),
  call_dcg__(RuleSet, Input, Rest, Memory, Context, Session, Params).
...
call_dcg__(RuleSet, Input, Rest, Memory, Context, Session, Params) :-
      (   strip_module(RuleSet, M, Plain),
          nonvar(Plain),
          dcg_special(Plain)
      ->  writeln("Processing special DCG rule: "), writeln(RuleSet),
          dcg_body(Plain, _, q(M,M,_), S0, S, Body, _),
          Input = S0, Rest = S,
          call(M:Body)
      ;   
      
        writeln("Processing DCG rule: "), writeln(RuleSet),
        
        RuleSet =.. L,
        L  = [:,_,Goal0], 
        Goal0 =.. LL,
        LL = [Goal1|Args],
        append(Args, [Input, Rest], NewArgs),
        NewGoal =.. [Goal1|NewArgs],
        NewGoal2 =.. [:,Session,NewGoal],
        
        writeln("Calling DCG rule: "), writeln(NewGoal2),
...
mi(Goal, Memory, Context, Session, Params) :-
 ...
 mi(Goal, Memory, Context, Session, Params) :-
  ...
  Goal \= read_file_to_string(_,_,_),
   ...
   writeln(check_is_user_rule(Goal)),
  mi_check_user_rule(Goal, Session),
